...
export default {
    ...
    data() {
        return {
            //专辑logo url
            album_str: "https://y.gtimg.cn/music/photo_new/T002R300x300M000",
            album_suffix: ".jpg?max_age=2592000",

            isPlaying: false,
            currentSong: {},
            songData: this.$route.params,
            songIndex: 0,
            percent: 0,
            songDuration: 0,
            voice: 100,
    ...
    created() {
        let tempArr = JSON.parse(sessionStorage.getItem('songList'));
        console.log(tempArr);
    ...
    mounted() {
        ...
        axios.get("/api/getSongList", {
            params: {
                id: _this.songData.songmid
            }
        })
            .then(response => {
                var data = response.data[0];
                data.song_name = _this.songData.songname;
                data.artist_name = _this.songData.singer[0].name;
                data.album_logo = _this.album_str + _this.songData.albummid + _this.album_suffix;
                data.lyric = null;
                if (data) {
                    _this.songList.push(data);
                }
            })
    ...
    watch: {
        ...
        songList: {
            handler() {
                sessionStorage.setItem('songList', JSON.stringify(this.songList));
            },
            deep: true
        }
    }
